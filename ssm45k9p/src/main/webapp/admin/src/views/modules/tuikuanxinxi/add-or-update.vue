<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="退款日期" prop="tuikuanriqi">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.tuikuanriqi" 
                type="datetime"
                placeholder="退款日期">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.tuikuanriqi" label="退款日期" prop="tuikuanriqi">
              <el-input v-model="ruleForm.tuikuanriqi" 
                placeholder="退款日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.kechengtupian = true;
          ...
          if(o=='tuikuanriqi'){
            this.ruleForm.tuikuanriqi = obj[o];
	    this.ro.tuikuanriqi = true;
            continue;
          }
          if(o=='tuikuanyuanyin'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.huiyuankahao!=''&&json.huiyuankahao){
              		this.ruleForm.huiyuankahao = json.huiyuankahao
		}
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tuikuanxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tuikuanxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
